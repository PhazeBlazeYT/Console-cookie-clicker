(() => {
  ...
  const prestigePotential = () => Math.floor(Math.sqrt(state.totalCookiesBaked / 1000000));
  const prestigeGain = () => Math.max(0, prestigePotential() - state.prestigeChips);
  const prestigeNeedForNext = () => {
    const nextTarget = (prestigePotential() + 1) ** 2 * 1000000;
  ...
  style.textContent = `
    ...
    #${ROOT_ID} .cc-bg { position:absolute; inset:-6%; background:
      radial-gradient(120% 120% at 20% 10%, #7a4a2e 0%, #3c2a1f 42%, #2a2430 70%, #2f241b 100%),
    ...
    #${ROOT_ID} .cc-mid { padding:12px; background:linear-gradient(180deg,#3d2c38d9,#1a1417d9); display:flex; flex-direction:column; gap:10px; border-left:1px solid #f2d3ff33; border-right:1px solid #f2d3ff22; }
    ...
    #${ROOT_ID} .cc-scene { border:1px solid #0008; background:linear-gradient(180deg,#2d1e21,#1a1310); padding:8px; min-height:120px; box-shadow: inset 0 1px 0 #fff1; }
    ...
    #${ROOT_ID} .cc-golden::after { content:'Golden'; position:absolute; bottom:-18px; left:50%; transform:translateX(-50%); font-size:11px; color:#ffe7a8; text-shadow:0 1px 2px #000; }
  ...
  function renderStats() {
    ...
    els.prestigeStat.innerHTML = policy.createHTML(
      ...
      `Clicks: <b>${fmt(state.totalClicks)}</b> | Golden clicks: <b>${fmt(state.goldenClicks)}</b> / spawns: <b>${fmt(state.goldenSpawns)}</b> | Achievements: <b>${state.achievements.length}/${achievementDefs.length}</b><br>` +
  ...
  function spawnGoldenCookie() {
    ...
    const wrath = Math.random() < 0.16;
    if (wrath) gc.classList.add('cc-wrath');
    gc.textContent = wrath ? '😈' : '🍪';
    ...
    gc.onclick = () => {
      ...
      if (wrath && r < 0.28) {
        ...
        const chain = Math.max(7, Math.floor((Math.log10(Math.max(1, state.cookies)) + 1) * 111));
        state.cookies += chain;
